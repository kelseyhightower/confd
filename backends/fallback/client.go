...
func NewFallbackClient(mainBackend, fallbackBackend StoreClient) (*FallbackClient, error) {
	return &FallbackClient{mainBackend, fallbackBackend}, nil
}

// GetValues queries the environment for keys
func (c *FallbackClient) GetValues(keys []string, token string) (map[string]string, error) {
	fallback_vars, err := c.fallbackBackend.GetValues(keys, token)
	if err != nil {
		return nil, err
	}

	vars, err := c.mainBackend.GetValues(keys, token)
	if err == nil {
