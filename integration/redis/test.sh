...
confd --onetime --log-level info --confdir ./integration/confdir --interval 5 --backend redis --node 127.0.0.1:6379
if [ $? -ne 0 ]; then
        exit 1
fi

confd --onetime --log-level info --confdir ./integration/confdir --interval 5 --backend redis --node 127.0.0.1:6379/0
if [ $? -ne 0 ]; then
        exit 1
fi
