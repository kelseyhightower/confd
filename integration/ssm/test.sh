...
confd --onetime --log-level debug --confdir ./integration/ssm --interval 5 --backend ssm --table confd
if [ $? -ne 0 ]
then
        exit 1
fi
...
confd --onetime --log-level debug --confdir ./integration/ssm --interval 5 --backend ssm --table confd --watch
if [ $? -eq 0 ]
then
        exit 1
fi
