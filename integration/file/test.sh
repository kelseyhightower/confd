...
mkdir ${TDIR}/backends1 ${TDIR}/backends2
cat <<EOT >> ${TDIR}/backends1/1.yaml
key:
  key: foobar
database:
  host: 127.0.0.1
...
cat <<EOT >> ${TDIR}/backends1/2.yaml
upstream:
  app1: 10.0.1.10:8080
  app2: 10.0.1.11:8080
EOT

cat <<EOT >> ${TDIR}/backends2/1.yaml
nested:
  app1: 10.0.1.10:8080
  app2: 10.0.1.11:8080
EOT

cat <<EOT >> ${TDIR}/backends2/2.yaml
prefix:
  database:
    host: 127.0.0.1
    password: p@sSw0rd
...
# Run confd
confd --onetime --log-level debug --confdir ./integration/confdir --backend file --file ${TDIR}/backends1/ --file ${TDIR}/backends2/  --watch
